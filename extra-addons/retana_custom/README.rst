...
- Agrega una hoja de estilos (``static/src/css/retana_dark_theme.css``) al bundle
  ``web.assets_backend``, que aplica un tema oscuro a la navbar, dropdowns y
  controles de formulario del backend.
...
- Se removieron imports de ``@odoo/owl`` (``Component``, ``onMounted``) que no se
  usaban: el archivo no define un componente OWL, solo JS plano que se ejecuta en
  ``DOMContentLoaded``.
- Se eliminó ``static/src/css/retana_dark_theme copy.css``, un archivo de respaldo
  desactualizado que no estaba referenciado en ningún lado y quedó versionado por
  error.
